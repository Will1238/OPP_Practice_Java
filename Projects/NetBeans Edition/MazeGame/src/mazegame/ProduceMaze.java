...
public class ProduceMaze {
    ...
    /**
     * 初始化方法，在本类的构造方法和ControlWindow类的refresh按钮的单击事件中调用
     */
    public void init(){
        steps = 0;
        currentPoint = new Point(startPoint.getX(), startPoint.getY());
    ...
    /**
     * 用来存储当前迷宫
     * @param savePath 存储路径，由ControlWindow的对话框传递的字符串作为参数
     * @return 当用户单击对话框上的取消时会传回一个null引用，此时无法打开文件，返回false，正常打开返回true
     * @throws IOException 
     */
    public boolean saveMaze(String savePath) throws IOException{
        if(savePath == null){
            return false;
        }
    ...
    /**
     * 用来读取指定迷宫
     * @param loadPath 由ControlWindow的对话框传递的字符串作为参数
     * @return 意义同saveMaze
     * @throws IOException 
     */
    public boolean loadMaze(String loadPath) throws IOException{
        if(loadPath == null){
            return false;
        }
